...
Run: uv run python thesis/presentations/workshop_may_2026/extend_with_act2_act3.py
"""
...
SRC = PROJECT / 'results/regressions/b9_replicated_isp_grain.csv'
...
PROJECT = Path.cwd() if (Path.cwd() / 'data').exists() else Path.cwd().parent.parent
full = pd.read_csv(PROJECT/'results/regressions/b9_replicated_isp_grain.csv')
apr_sep = pd.read_csv(PROJECT/'results/regressions/b9_replicated_isp_apr_sep.csv')
...
PROJECT = Path.cwd() if (Path.cwd() / 'data').exists() else Path.cwd().parent.parent
rt2 = pd.read_csv(PROJECT/'results/regressions/rt2_post_blackout/01_perfirm_perregime.csv')
REGIMES = ['pre-IDA', '3-sess', 'ISP15-win', 'DA60/ID15', 'DA15/ID15']
BIG4 = ['GE', 'IB', 'GN', 'HC']
COLORS = {'GE': '#d62728', 'IB': '#1f77b4', 'GN': '#2ca02c', 'HC': '#9467bd'}
