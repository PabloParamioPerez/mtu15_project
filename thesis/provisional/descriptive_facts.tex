...
\noindent The cost cascade (\S\ref{sec:prov:full_cascade}) shifts expenditure between real-time technical restrictions and pre-IDA Fase~I; the bidding-side mirror is the continuous intraday market (XBID). If finer-grained intraday clearing absorbs within-hour scarcity, XBID repositioning volume should fall --- and it does. The per-tech, per-firm and per-regime breakdowns are summarised here, not reproduced in full.

\paragraph{Volume.} Gross XBID activity falls during MTU15-IDA and partly recovers under MTU15-DA --- deseasonalized $\sim 32 \to 12 \to 23$ GWh/day across the five regimes ($-29\%$ from the pre-reform baseline to DA15/ID15, trough $-62\%$ in the 40-day MTU15-IDA-only window) --- the mirror image of the granular auction absorbing within-hour scarcity that previously fell to XBID and real-time TR. CI's \emph{share} of OMIE-cleared volume traces the same path ($\sim 3\% \to 1.7\% \to 2.9\%$), back to its pre-reform level. Bilateral contracts stay the largest channel throughout ($\sim 60\%$ of dispatched volume), with renewables' bilateral share rising sharply under DA15/ID15 (Wind $20 \to 43\%$, Solar PV $15 \to 33\%$).

\paragraph{Direction and price.} CCGT is a systematic net \emph{seller} in XBID in every regime ($+0.8$ to $+2.5$ GWh/day) --- the continuous-market face of ``stay out of the day-ahead, re-enter via post-clearing channels'' --- and nuclear a systematic net \emph{buyer} ($-1.7$ to $-2.9$ GWh/day post-blackout), backing off as the grid favoured forced-on CCGTs. CCGT and Hydro sell in XBID at a small premium to the same-day day-ahead price ($+\sim\!\texteuro 6$/MWh, scarcity-side); Nuclear and Solar PV at a large discount ($-\texteuro 20$ to $-\texteuro 28$, excess-supply-side). The cross-tech price structure is stable across regimes; only volumes move.

\paragraph{Within-hour rebalancing.} Per firm, the within-hour dispersion of XBID sell volume across the four quarters of a clock-hour is $\sim 2$--$5\times$ larger in critical than flat hours --- the same critical-flat asymmetry as the auction-side bid-shape evidence --- and ranks IB $>$ GN $>$ HC $>$ GE. It is stable across the post-MTU15-IDA regimes: CI \emph{volume} fell with the granular auction, but per-firm within-hour rebalancing \emph{intensity} did not.
