...
\paragraph{The intraday auctions show it is strategy, not cost.} The pre-IDA Fase~I redispatch settles the CCGT grid-security role \emph{before} the intraday auctions clear (post-IDA RT on CCGT is negligible until DA15/ID15; \S\ref{sec:prov:phf_minus_pdbf}), so the intraday auction is a market in which the Fase~I-recall game is no longer in play. Figure~\ref{fig:ida_vs_da} puts each Big-4 CCGT fleet's day-ahead and intraday-auction supply curves side by side, and the contrast is total: in the day-ahead the curve is two-tier, with $19$--$75\%$ of MW withheld above the $\sim\!200$ ceiling; in the intraday auction the \emph{same fleets, same days} bid a single competitive tier around marginal cost --- a MW-weighted $82$--$96$ EUR/MWh and $\sim\!0\%$ withheld, for all four firms, Naturgy included ($90$ against its $767$ day-ahead). A unit's marginal cost does not change between two auctions held hours apart; the firm simply reveals it in the intraday one. \textbf{The day-ahead bid is therefore not a cost --- it is the price of \emph{not clearing}, so as to be recalled pay-as-bid in Fase~I.} This is the cleanest evidence in the document that the day-ahead two-tier curve is withholding rather than cost-reflective bidding.
...
\paragraph{Wind within-hour variation is forecast, not strategy.} A natural worry is that the quarter-to-quarter movement in wind bids is strategic scarcity-chasing rather than the wind forecast. It is not. We regress each wind unit's MW-weighted quarter bid price on a within-hour scarcity proxy --- the quarter's day-ahead load forecast net of solar (residual demand \emph{excluding} wind, hence exogenous to any wind unit), deviated from its clock-hour mean --- with unit$\times$date$\times$hour fixed effects, so only across-quarter within-hour variation identifies the slope (DA15/ID15, $1.1$M unit-date-quarter observations, date-clustered SEs). Three facts: (i) $86\%$ of wind clock-hours carry \emph{zero} within-hour price variation --- the unit submits one price across all four quarters; (ii) where price does vary, it is essentially uncorrelated with scarcity --- within-hour partial correlation $-0.03$ (and only $-0.07$ on the price-varying subsample), slope $\approx -0.05$ EUR/MWh per GW, economically nil and, if anything, the wrong sign for scarcity-chasing; (iii) the null is unchanged controlling for the unit's own within-hour forecast (offered-MW) shape, holds with plain load in place of load-minus-solar, and holds even for Gesternova, the one aggregator that ladder-bids across quarters. The within-hour movement in wind bids is the sub-hourly forecast, not strategic shaping --- which is why the wind functionals (\S\ref{sec:prov:per_curve_def}) are read as a price-taking baseline.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{../../figures/working/wind_within_hour_scarcity.pdf}
\caption{\textbf{Wind within-hour bid price does not respond to within-hour scarcity} (DA15/ID15, unit$\times$date$\times$hour fixed effects). Left: binned scatter of the within-hour-demeaned wind bid price against the within-hour-demeaned scarcity proxy (load minus solar forecast, GW); the fitted slope is flat. Right: the same slope estimated per wind aggregator with date-clustered $95\%$ intervals --- no aggregator, Gesternova included, shows an economically meaningful response.}
