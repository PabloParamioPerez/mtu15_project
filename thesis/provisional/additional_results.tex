...
\noindent The CCGT and Hydro\_pump results are interesting because these are the model's prototypical strategic-arbitrageur techs: thermal units that should arbitrage between DA and IDA based on price expectations, and pumped-storage that should do classical financial arbitrage. Their absence of an MTU15-IDA-induced increase suggests that the dual-direction count in the model's sense (a measure of \emph{rebalancing} more than \emph{profit-driven arbitrage}) is not where thermal/storage arbitrage shows up. Pure profit-arbitrage by thermal units may operate through net-direction changes across sessions (a phenomenon the measure misses, per the caveat above) rather than within-session dual-direction bidding. The CCGT scale-up at DA15 documented in \emph{preliminary results} §4.A.2 is the more direct empirical signature of thermal arbitrage --- through quantity migration into DA rather than through within-session position rebalancing in IDA. \textbf{Read together, the two findings characterise two distinct empirical arbitrage channels}: (a)~\emph{within-session forecast-driven rebalancing in IDA}, dominated by RES/biomass and amplified by MTU15-IDA; (b)~\emph{between-market quantity migration into DA}, dominated by CCGT and amplified by DA15.
