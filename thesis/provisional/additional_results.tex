...
\paragraph{Offer types: what the functionals see.} The functionals read the price-quantity tranche ladder a unit submits --- the \emph{simple offer}, in the offer-detail record. A unit may instead declare \emph{complex conditions}: a minimum-income condition (dispatch only if revenue clears a fixed-plus-variable floor), an indivisible first tranche, a load gradient, or a \emph{block order} (one price held across several periods, accepted whole). This is not innocuous: a near-zero $\sigma_p$ is genuine flat bidding for a pure simple offer, but for a minimum-income unit it can instead be strategy displaced onto the income floor; a block order contributes one price to every period it covers, mechanically forcing $\sigma_p \to 0$ and $N_{\text{eff}} \to 1$; an indivisible first tranche is a quantity lump, depressing $N_{\text{eff}}$ structurally. The offer-type mix is sharply tech-specific (DA15/ID15 sell offers): Wind, Solar, Hydro and pump-storage are $\sim\!100\%$ pure simple offers, so their functionals read at face value; Nuclear is $64\%$ simple ($36\%$ minimum-income); \textbf{CCGT is the outlier --- only $18\%$ simple, with $50\%$ carrying a minimum-income condition and $32\%$ a block order.} The functionals are clean for the renewable and hydro techs; for CCGT the step from geometry to \emph{strategic shaping} must be read conditional on offer type.
...
\paragraph{Pump-storage within-day arbitrage under MTU15-DA: the quantity-side trace.} The bid-shape evidence above is one side of the granular-pricing reform; the quantity-side mirror, after seasonality is absorbed, comes from pump-storage. Per-tech DA cleared-MW DiDs across the MTU15-DA cutover (critical vs flat, pre Jul--Sep vs post Oct--Dec 2025) look dramatic at the raw level --- Solar PV $-3\,282$ MW, CCGT $+1\,548$, others mixed --- but they are dominated by the Jul $\to$ Oct daylight-pattern seasonality (long-evening solar peak collapses into short-evening fall, CCGT and hydro rise to fill it). Adding the descriptive\_facts \S\ref{sec:bid_internal:calendar_discipline} Fourier basis interacted with the critical-hour indicator (4 annual harmonics $\times$ crit $+$ 6 DOW $\times$ crit, the date-FE-compatible form of the daily-aggregate SA) collapses essentially every per-tech DiD: Solar PV flips to $+730$ MW ($t=+2.81$), CCGT goes to $-343$ MW ($t=-0.30$, ns), run-of-river hydro to $+91$ ($t=+0.81$, ns), nuclear / coal / biomass / wind all null. \emph{Only Hydro pump survives} at $+482$ MW ($t=+2.87$). Reading the pre/post means on both sides of the pump-storage book makes the mechanism transparent:
...
\noindent Generation reallocates from flat hours ($-64\%$) to critical hours ($+20\%$); pumping mirrors it (critical $-52\%$, flat from near-zero to $566$ MW). \emph{Total daily generation is essentially unchanged} ($14\,874 \to 14\,250$ MWh, $-4\%$). This is textbook within-day arbitrage: pre-MTU15-DA, hourly bids could not price-discriminate across quarters and the pump/turbine schedule was messy; post-reform, the unit class with the largest within-hour arbitrage value exploits within-hour price spreads cleanly. It is the cleanest causal trace of MTU15-DA on the quantity side, and the only per-tech cleared-MW DiD robust to the Fourier $\times$ crit deseasonalisation. Source: \texttt{scripts/analysis/bid/fourier\_sa\_cleared\_mw\_did.py}.
