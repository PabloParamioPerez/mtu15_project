...
> Spain's 2024–2025 reform sequence revealed **two distinct mechanism-design failures**, each requiring a separate policy lever. (1) **Asymmetric clock scales** caused a €1.1B BRP→TSO settlement transfer in the 10-month asymmetric-granularity window (Dec 2024 – Sep 2025). Lever 1: clock-symmetry at MTU15-DA. *Implemented* — the transfer collapsed 6× (€91M/mo → €7.4M/mo) when DA aligned to 15-min. (2) **Non-Pigouvian uniform-rate allocation across heterogeneous-marginal-cost segments** redistributes burden from dispatchable plants to renewable-portfolio firms. Lever 2: settlement-rule redesign. *Open* — wind + LIB free-market retailers consistently pay 60–65% of imbalance settlement € in EVERY post-ISP15 regime, including post-MTU15-DA. Clock-symmetry shrinks the *scale* of the redistribution but does NOT fix its *structure*. The 2025-04-28 Iberian blackout amplifies but does not create either friction.
...
| 12 | What this DOESN'T show | — | F7 (€820M IB DA rent), F8 (Bushnell-style hydro Q4 dispatch), F22 (CNMC SBO3 fleet-wide wedge) — REGIME-INVARIANT background, covered in Parts II–IV of thesis | Honest scoping: "we have firm-level market-power findings; they're not reform-driven and belong in a separate paper / thesis chapter" |
...
A: Blackout-split robustness (Fig 5): clean April 2025 PRE-blackout single month is +€75.7M; post-blackout DA60/ID15 averages €93.5M/mo (only +24% above clean April — modest amplification, not source); and crucially **the post-MTU15-DA collapse holds at €7.4M/mo despite operación reforzada still in effect**. The collapse decisively separates granularity friction from blackout effects.
...
A: We're explicit that this is reduced-form. Same-calendar-month pre-IDA baseline + cross-country placebo + bootstrap robustness. No causal identification at the firm-level (the synthetic-firm method is structural-comparative, not causal). The Brodersen-style Bayesian structural time series mentioned in Feb-deck slide 9 is a candidate for the thesis main analysis but not used here.
...
The May talk is **Part I only**. Full thesis structure (in [`thesis/drafts/master_thesis_proposal.md`](../drafts/master_thesis_proposal.md)) covers 37 alive findings across:
...
**Building the slide deck (Beamer):** the figures live at [`thesis/figures/fig01..fig07.{pdf,png}`](../figures/) — 7 PDFs ready for direct `\includegraphics{...}` in Beamer. The IO framing in this guide can be directly transcribed into slide markdown. Mirror the file naming convention from `presentation1/` (`Paramio_Pablo_slides_may2026.tex` and `.pdf`).
...
```bash
uv run python thesis/presentation2/build_figures.py
