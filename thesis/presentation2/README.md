...
Presentation-deliverable source artefacts are kept apart from `explore/` (exploratory notebooks) and `scripts/analysis/` (per-claim analysis scripts) so that "what produces the slides" is unambiguous. `thesis/figures/` stays shared at thesis-level because the same figures appear in both the May presentation and Part I of the thesis chapters; duplicating per-deliverable would create drift.
