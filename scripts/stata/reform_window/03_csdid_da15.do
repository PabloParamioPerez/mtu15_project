* ============================================================================
* 03_csdid_da15.do  --  DR-DiD on the DA15 reform window for ALL outcomes
* INPUT:  panels/da15_{da,ida}_spread.dta  (one row per date; price_crit + price_flat)
* OUTPUT: thesis/paper/tables/reform_window/tab_csdid_da15_{da,ida}.tex
*         coefs/csdid_da15_{da,ida}.csv
*         logs/03_csdid_da15.log
* Method: Two 2x2 DR-DiDs per outcome (one on critical-hour mean price, one on
*         flat-hour mean price), DDD = ATT_crit - ATT_flat. Joint SE from
*         influence-function difference (drdid stub() option).
* ============================================================================

do "${REPO}/scripts/stata/reform_window/_config.do"

log using "${LOGS}/03_csdid_da15.log", text replace
...
foreach mkt in da ida {
    ...
    * DDD via influence-function difference
    ...
    file write f "\caption{DR-DiD on `outcome' in DA15 window, prices in levels (Sant'Anna--Zhao 2020). 2$\times$2 DiD per hour-class, covariates \texttt{i.dow + i.month}; DDD is the difference of ATTs with joint SE from the influence-function difference.}\label{tab:reform_csdid_`window'_`mkt'}" _n
    ...
    file write f "Window               & DA15 (reform = 2025-10-01) & \\\\" _n
