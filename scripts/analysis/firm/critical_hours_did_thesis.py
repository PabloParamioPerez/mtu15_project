...
CRITICAL_HOURS = (18, 19, 20, 21, 22)  # price_peak (canonical, per calibration memo)
FLAT_HOURS = (3, 4, 5)
...
def build_panel(units: pd.DataFrame) -> pd.DataFrame:
    ...
    panel["crit"] = (panel["hour_class"] == "critical_h18_22").astype(int)
    ...
    panel = panel[panel["hour_class"].isin(["critical_h18_22", "flat_h3_5"])].copy()
    return panel
