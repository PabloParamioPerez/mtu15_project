...
# FEEDS: B2, B5
# CLAIM: Build per-firm DA+IDA bid-level + revenue panels for Big-4
"""Build per-firm per-day bid-level + revenue panels for P2 + P5.

Outputs:
  data/derived/panels/firm_bid_panel.parquet  (DA bid-weighted avg offer price per firm×day)
  data/derived/panels/firm_revenue_panel.parquet  (DA + IDA revenue per firm×day)
"""
...
con.execute("""
    COPY (
        ...
        GROUP BY 1, 2
        ORDER BY 1, 2
    ) TO 'data/derived/panels/firm_bid_panel.parquet' (FORMAT PARQUET)
""")
...
con.execute("""
    COPY (
        ...
        ORDER BY date, firm
    ) TO 'data/derived/panels/firm_revenue_panel.parquet' (FORMAT PARQUET)
""")
...
stats = con.execute("""
    ...
    FROM 'data/derived/panels/firm_revenue_panel.parquet'
