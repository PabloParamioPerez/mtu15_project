...
# CLAIM: Build a per-(firm, tech, date) daily post-DA gap panel (PHF_last -
#        PDBC, GWh/day) with a SA column attached. Per-(firm, tech) FWL
#        regression on gap_gwh ~ const + regime + Fourier(doy, K=4) + DOW
#        (identity link, since the gap is signed). Per-cell SA strips Fourier
#        + DOW deviation and keeps regime + idiosyncratic residual.
#
# OUT: data/derived/panels/post_da_gap_sa_daily.parquet
#      columns: d, firm, tech, gap_gwh, gap_gwh_sa
...
OUT = OUT_DIR / "post_da_gap_sa_daily.parquet"
...
def build_raw_panel():
    """Per-(date, firm, tech) daily post-DA gap (GWh)."""
...
def per_firm_tech_sa(group, fourier_cols, dow_cols, regime_cols):
    """Identity-link FWL on gap_gwh; return group with gap_gwh_sa column."""
...
def main():
    print("Building raw daily post-DA gap panel...")
    df = build_raw_panel()
    ...
    print(f"Fitting per-(firm, tech) FWL-SA (identity link, K={K}, MIN_OBS={MIN_OBS})...")
    pieces = []
    for _, g in df.groupby(["firm", "tech"], sort=False):
        pieces.append(per_firm_tech_sa(g, fourier_cols=fourier_cols,
                                       dow_cols=dow_cols, regime_cols=regime_cols))
    out = pd.concat(pieces, ignore_index=True)
    n_sa = out["gap_gwh_sa"].notna().sum()
    print(f"  {n_sa:,} / {len(out):,} cells with SA gap")

    out_cols = ["d", "firm", "tech", "gap_gwh", "gap_gwh_sa"]
