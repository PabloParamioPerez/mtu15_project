...
def main() -> None:
    ...
    if len(mtu60) > 0:
        mtu60["qida_mwh"] = mtu60["qida_mwh"] / 4.0
        ...
        mtu60_exp = rep[["date", "period", "mtu_minutes", "firm", "qida_mwh", "was_replicated"]]
    else:
        mtu60_exp = pd.DataFrame()

    mtu15["was_replicated"] = False
    mtu15["hour"] = ((mtu15["period"].astype(int) - 1) // 4) + 1
    mtu15_full = mtu15[["date", "period", "mtu_minutes", "firm", "qida_mwh", "was_replicated", "hour"]]
    ...
    if len(da60) > 0:
        da60["qda_sell_mwh"] = da60["qda_sell_mwh"] / 4.0
        ...
        rep["mtu_minutes"] = 15
        da60_exp = rep[["date", "period", "firm", "qda_sell_mwh"]]
    else:
        da60_exp = pd.DataFrame(columns=["date", "period", "firm", "qda_sell_mwh"])
    da15_use = da15[["date", "period", "firm", "qda_sell_mwh"]]
    da_isp = pd.concat([da60_exp, da15_use], ignore_index=True)
    da_isp = da_isp.groupby(["date", "period", "firm"], as_index=False)["qda_sell_mwh"].sum()
