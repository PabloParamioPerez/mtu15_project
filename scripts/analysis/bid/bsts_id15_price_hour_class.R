...
#        BROKEN OUT BY HOUR-CLASS, both DA and IDA, reforzada-constant
#        windows, base spec (wind+solar+gas). Tells us where the residual
#        -24.7 EUR/MWh DA placebo-net price drop actually lives.
#
# OUT: results/regressions/bid/mtu15_critical_flat/bsts_id15_price_hour_class.csv
...
sp <- if (length(sa)) sub("^--file=", "", sa) else
  "scripts/analysis/bid/bsts_id15_price_hour_class.R"
repo <- normalizePath(file.path(dirname(sp), "..", "..", ".."))
panel_fp <- file.path(repo, "data/derived/panels/price_hour_class_panel.parquet")
out_dir <- file.path(repo, "results/regressions/bid/mtu15_critical_flat")

COVARS <- c("wind_gwh", "solar_gwh", "gas_eur")
OUTCOMES <- c("da_p_critical", "da_p_midday", "da_p_flat",
              "ida_p_critical", "ida_p_midday", "ida_p_flat")
...
for (cfg in CFGS) {
  ...
  for (outcome in OUTCOMES) {
    ...
    if (is.null(r)) next
    cat(sprintf("  %-15s eff=%+7.2f  [%+6.2f, %+6.2f]  p=%5.3f\n",
                outcome, r$eff, r$lo, r$hi, r$p))
...
write.csv(out_df, file.path(out_dir, "bsts_id15_price_hour_class.csv"),
           row.names=FALSE)
cat(sprintf("\nWrote %d rows\n", nrow(out_df)))
