...
# FEEDS: thesis/paper/thesis.tex appendix sec:5-nuclear-pdbf -- BSTS on
#        daily nuclear PDBF (auction + bilateral) and PHF (final post-IDA)
#        under the headline Spec A methodology (long pre-window
#        2022-01-01 onwards + year-by-year wind and solar interactions).
#
# Outcomes: nuclear_pdbf_gwh, nuclear_phf_gwh
# Reform windows mirror the Spec A windows:
...
#
# OUT: results/regressions/bid/mtu15_critical_flat/bsts_nuclear_pdbf.csv
...
panel_fp <- file.path(repo, "data/derived/panels/nuclear_pdbf_phf_daily.parquet")
out_dir  <- file.path(repo, "results/regressions/bid/mtu15_critical_flat")

BASE_COVARS <- c("wind_gwh", "solar_gwh", "gas_eur")
OUTS <- c("nuclear_pdbf_gwh", "nuclear_phf_gwh")
...
run_bsts <- function(panel, response, pre_start, post_start, post_end, tag) {
  ...
  cat(sprintf("  %-40s eff=%+9.3f  CI=[%+9.3f,%+9.3f]  p=%.4f  n=%d/%d\n",
              tag, eff, lo, hi, pv,
...
out_fp <- file.path(out_dir, "bsts_nuclear_pdbf.csv")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
write.csv(out, out_fp, row.names = FALSE)
cat(sprintf("\nWrote %s with %d rows.\n", out_fp, nrow(out)))
print(out)

# Placebo-net computation
cat("\n=== Placebo-net summary ===\n")
for (reform in c("ID15", "DA15")) {
  for (outc in OUTS) {
    ...
    if (!is.na(real) && !is.na(plac)) {
      cat(sprintf("  %-5s %-20s real=%+8.2f  placebo=%+8.2f  net=%+8.2f\n",
                  reform, outc, real, plac, real - plac))
    }
  }
}
