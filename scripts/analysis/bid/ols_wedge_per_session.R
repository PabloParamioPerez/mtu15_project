...
# FEEDS: thesis/paper/thesis.tex --- OLS on the DA-IDA wedge, per session and
#        aggregate. Wedge = DA price - IDA price for each session.
#        Same five-spec ladder as ols_price_full_controls.R.
#
# Headline year-by-renewable spec:
#   - ID15 headline = Spec 4 (2024+2025 pooled).
#   - DA15 headline = Spec 3 (per-year separate).
#
# OUT: results/regressions/bid/mtu15_critical_flat/ols_wedge_per_session.csv
...
WEDGES <- c("wedge_agg", "wedge_s1", "wedge_s2", "wedge_s3")
specs <- list()
i <- 1
for (w in WEDGES) {
  # ID15: long-pre OK for wedge_agg; per-session restricted to 2024-06-14+
  pre_lo_id15 <- ifelse(w == "wedge_agg", "2022-01-01", "2024-06-14")
  pre_lo_da15 <- ifelse(w == "wedge_agg", "2022-01-01", "2024-06-14")
  specs[[i]] <- list(tag=sprintf("ID15 %s", w), response=w,
                     pre_lo=pre_lo_id15, post_lo="2025-03-19", post_hi="2025-04-27"); i <- i+1
  specs[[i]] <- list(tag=sprintf("DA15 %s", w), response=w,
                     pre_lo=pre_lo_da15, post_lo="2025-10-01", post_hi="2025-12-31"); i <- i+1
}
all_rows <- do.call(rbind, lapply(specs, function(s)
  run_ols(s$response, s$pre_lo, s$post_lo, s$post_hi, s$tag)))
write.csv(all_rows, file.path(repo, "results/regressions/bid/mtu15_critical_flat/ols_wedge_per_session.csv"),
          row.names=FALSE)
cat("\nWrote ols_wedge_per_session.csv\n")
