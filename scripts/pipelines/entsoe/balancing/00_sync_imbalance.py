...
CATEGORY = "imbalance"
KIND_DOC_TYPE = {
    "prices": DOC_TYPE["imbalance_prices"],
    "volumes": DOC_TYPE["imbalance_volumes"],
}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Download ENTSO-E imbalance documents for Spain, one XML per "
            "month (idempotent with recent-month refresh)."
        )
    )
    ...
    p.add_argument(
        ...
        help=(
            ...
            "ENTSO-E may publish late-arriving revisions. Default: 2."
        ),
    )
    return p.parse_args()
...
def main() -> None:
    ...
    file_family = f"imbalance_{args.kind}"
    raw_dir = PROJECT_ROOT / "data/raw/entsoe/balancing/imbalance" / args.kind
    ...
    print(
        ...
        f"documentType:  {KIND_DOC_TYPE[args.kind]}\n"
        f"Raw dir:       {raw_dir}\n"
    ...
    for i, (yyyymm, ps, pe) in enumerate(chunks):
        ...
        params = {
            "documentType": KIND_DOC_TYPE[args.kind],
        ...
        append_csv_row(
            manifest_csv,
            {
                ...
                "source_url": f"entsoe:A{KIND_DOC_TYPE[args.kind][1:]}"
