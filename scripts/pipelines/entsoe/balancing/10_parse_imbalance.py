...
from mtu.parsing.entsoe.imbalance import parse_file  # noqa: E402
...
CATEGORY = "imbalance"
PARSER_NAME = "entsoe.imbalance.parse_file"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Parse ENTSO-E imbalance XMLs into per-month Parquet."
    )
    p.add_argument("--kind", required=True, choices=["prices", "volumes"])
    p.add_argument(
        "--overwrite",
        action="store_true",
        help="Re-parse files even if their output already exists.",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    for raw_path in files:
        ...
        try:
            df = parse_file(raw_path, kind=args.kind)
            rows_output = len(df)
            if df.empty:
                counts["empty"] += 1
                # Do not write a zero-column parquet — DuckDB cannot read it.
                if out_path.exists():
                    out_path.unlink()
            else:
