...
DOCUMENT_TYPE = DOC_TYPE["wind_solar_forecast"]  # A69 (same docType as DA)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Download ENTSO-E intraday (current) wind & solar forecast "
            "(A69 + processType A18) for Spain, one XML per month. "
            "Represents the most recent forecast issued before each MTU."
        )
    )
...
def main() -> None:
    ...
    print(
        ...
        f"processType:   {PROCESS_TYPE_INTRADAY}\n"
        f"Raw dir:       {raw_dir}\n"
    )
    ...
    for i, (yyyymm, ps, pe) in enumerate(chunks):
        ...
        params = {
            ...
            "processType": PROCESS_TYPE_INTRADAY,
