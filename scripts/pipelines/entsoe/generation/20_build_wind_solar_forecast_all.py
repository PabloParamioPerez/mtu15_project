...
def main() -> None:
    ...
    output_path = processed_dir.parent / "wind_solar_forecast_all.parquet"
    ...
    con.execute(f"""
        COPY (
            ...
            ORDER BY isp_start_utc, psr_type, position
        )
    ...
    psr_dist = con.execute(f"""
        SELECT psr_type, COUNT(*) AS n
        FROM read_parquet('{output_path}')
    ...
    print("Rows by psrType:")
    print(psr_dist.to_string(index=False))
