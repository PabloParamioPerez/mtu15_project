...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Download and extract OMIE ICAB monthly ZIP archives (idempotent with recent-month refresh)"
    )
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        for info in zf.infolist():
            ...
            if name.startswith("."):
                ...
                continue
            # Files inside the zip are uppercase (ICAB_YYYYMMDD.S)
...
def main() -> None:
    ...
    raw_dir = PROJECT_ROOT / "data/raw/omie/mercado_intradiario_subastas/ofertas/icab"
