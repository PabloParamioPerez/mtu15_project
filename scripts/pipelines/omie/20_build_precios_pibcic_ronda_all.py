...
def main() -> None:
    ...
    dup_mask = df.duplicated(subset=["date", "round_number", "period"], keep=False)
    dup_rows = int(dup_mask.sum())
    dup_keys = int(df.loc[dup_mask, ["date", "round_number", "period"]].drop_duplicates().shape[0])

    if dup_rows > 0:
        print(
            f"WARNING: Found {dup_rows} duplicated rows across {dup_keys} (date, round_number, period) keys "
            f"(likely multiple versions)."
        )
        print("Keeping latest version by version_suffix for each (date, round_number, period).")

    df = (
        df.sort_values(["date", "round_number", "period", "version_suffix_num"])
          .drop_duplicates(subset=["date", "round_number", "period"], keep="last")
