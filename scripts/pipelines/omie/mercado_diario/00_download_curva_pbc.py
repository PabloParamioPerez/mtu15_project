...
PARENTS_VALUE = "curva_pbc"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Download OMIE CURVA_PBC daily files")
...
def filename_for(d: date, version: int) -> str:
    return f"curva_pbc_{d.strftime('%Y%m%d')}.{version}"
...
def main() -> None:
    ...
    raw_dir = PROJECT_ROOT / "data/raw/omie/mercado_diario/curvas/curva_pbc"
    ...
    print(f"Downloading CURVA_PBC from {start} to {end} (max-version={args.max_version})")
