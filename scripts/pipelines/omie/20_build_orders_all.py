...
# (trade_date, source_file, row_number_in_file) is the natural unique key:
# each file is a snapshot for one day, row_number is per-file sequential.
IDENTITY_KEY = ["source_file", "row_number_in_file"]


def main() -> None:
    processed_dir = PROJECT_ROOT / "data/processed/omie/mercado_intradiario_continuo/ofertas/orders"
    output_path = processed_dir.parent / "orders_all.parquet"
    ...
    if n_dups > 0:
        ...
        """).df()
        raise ValueError(
            f"Orders raw table: {n_dups} duplicate key(s) on {IDENTITY_KEY}.\n"
