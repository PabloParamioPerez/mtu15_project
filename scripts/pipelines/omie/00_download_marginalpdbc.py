...
PARENTS_VALUE = "marginalpdbc"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Download OMIE MARGINALPDBC files")
...
def filename_for(d: date, version: int) -> str:
    return f"marginalpdbc_{d.strftime('%Y%m%d')}.{version}"
...
def main() -> None:
    ...
    print(f"Downloading MARGINALPDBC from {start} to {end} (max-version={args.max_version})")
    ...
    for d in daterange(start, end):
        ...
        for version in range(1, args.max_version + 1):
            ...
            # Sometimes endpoints return HTML error pages with status 200
