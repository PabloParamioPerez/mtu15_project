...
PARENTS_VALUE = "trades"

MARKET = "mercado_intradiario_continuo"
CATEGORY = "transacciones"
FILE_FAMILY = "trades"

# Confidentiality period: 90 days. Files older than 90 days from today are public.


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Download and extract OMIE TRADES monthly ZIP archives (idempotent). "
            "trades_YYYYMMDD.v files contain individual matched transactions in the "
            "continuous intraday market (§5.3.2.7). Subject to 90-day confidentiality."
        )
    )
...
def download_zip(
    *,
    ...
    if r.status_code == 404:
        print(f"[NO ZIP] {fname} (HTTP 404 — may be within 90-day confidentiality window)")
    ...
    if looks_like_html(content):
        print(f"[WARN ZIP] {fname} -> HTML response (likely 404 page), skipping")
        return "error", None
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        for info in zf.infolist():
            ...
            if name.startswith("."):
                ...
                continue
            if not name.lower().startswith(f"{FILE_FAMILY}_"):
...
def main() -> None:
    ...
    raw_dir = PROJECT_ROOT / "data/raw/omie/mercado_intradiario_continuo/transacciones/trades"
    ...
    print(
        ...
        f"Note: 90-day confidentiality — months within 90 days of today will return 404.\n"
        f"Raw dir:      {raw_dir}\n"
