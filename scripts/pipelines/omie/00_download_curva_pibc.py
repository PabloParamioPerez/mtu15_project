...
# On 2025-06-07 OMIE switched the session component in the filename from
# zero-padded two digits (01–06) to a single digit (1–6).
SESSION_FORMAT_CUTOFF = date(2025, 6, 7)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Download OMIE CURVA_PIBC daily files (all sessions)")
...
def filename_for(d: date, session: int, version: int) -> str:
    sess_str = f"{session:02d}" if d < SESSION_FORMAT_CUTOFF else str(session)
    return f"curva_pibc_{d.strftime('%Y%m%d')}{sess_str}.{version}"
...
def main() -> None:
    ...
    print(
        f"Downloading CURVA_PIBC from {start} to {end} "
    ...
    for d in daterange(start, end):
        ...
        for sess in range(1, args.max_sessions + 1):
            ...
            if not any_found_for_session:
                # Session not found for this date — normal for days before that session existed
                pass
