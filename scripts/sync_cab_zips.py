...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Download and extract OMIE CAB monthly ZIP archives (idempotent with recent-month refresh)"
    )
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        for info in zf.infolist():
            ...
            if name.startswith("."):
                ...
                continue
            # Files inside the zip are uppercase (CAB_YYYYMMDD.V)
...
def main() -> None:
    ...
    raw_dir = PROJECT_ROOT / "data/raw/omie/mercado_diario/ofertas/cab"
