...
FILENAME_RE = re.compile(r"^phfc_(\d{8})(\d{2})\.(\d+)$")


def parse_filename_metadata(path: Path) -> dict:
    m = FILENAME_RE.match(path.name)
    if not m:
        raise ValueError(f"Unexpected filename format for phfc: {path.name}")
...
def parse_phfc_file(path: Path) -> pd.DataFrame:
    ...
    with path.open("r", encoding="latin-1", errors="replace") as f:
        for i, raw_line in enumerate(f):
            ...
            if i == 0:
                if line != "PHFC;":
                    raise ValueError(f"{path.name}: expected first line 'PHFC;', got {line!r}")
                continue
