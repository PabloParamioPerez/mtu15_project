...
FILENAME_RE = re.compile(r"^omanulaintra_(\d{8})(\d{2})\.(\d+)$")
...
def parse_omanulaintra_file(path: Path) -> pd.DataFrame:
    """
    Parse one OMIE OMANULAINTRA raw file.

    Format:
      OMANULAINTRA;
      año;mes;dia;hora_emision;minuto_emision;sesion;version;  <- metadata row (skip)
    ...
    # content_lines[1] = metadata row (7 fields, some may be empty)
...
def parse_folder_and_write(
    ...
    for path in raw_files:
        ...
        if out_path.exists():
            ...
            continue
        try:
            df = parse_omanulaintra_file(path)
