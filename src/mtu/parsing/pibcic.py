...
FILENAME_RE = re.compile(r"^pibcic_(\d{8})(\d{2})\.(\d+)$")


def parse_filename_metadata(path: Path) -> dict:
    m = FILENAME_RE.match(path.name)
    if not m:
        raise ValueError(f"Unexpected filename format for pibcic: {path.name}")
...
def parse_pibcic_file(path: Path) -> pd.DataFrame:
    ...
    with path.open("r", encoding="latin-1", errors="replace") as f:
        for i, raw_line in enumerate(f):
            ...
            if i == 0:
                if line != "PIBCIC;":
                    raise ValueError(f"{path.name}: expected first line 'PIBCIC;', got {line!r}")
                continue
            ...
            data_row_counter += 1
            yyyy, mm, dd, period, round_number, unit_code, assigned_power, unused_zero, offer_type = parts
    ...
    for path in files:
            ...
            summary_rows.append(
                {
                    ...
                    "rows_output": 0,
                    "output_path": "",
                    "error_message": "Filename does not match pibcic pattern",
                }
            )
            continue
        ...
        try:
            ...
            row = {
                ...
                "file_family": "pibcic",
                "filename": path.name,
                "parser_name": "mtu.parsing.pibcic.parse_pibcic_file:v1",
