...
FILENAME_RE = re.compile(r"^pibcie_(\d{8})(\d{2})\.(\d+)$")


def parse_filename_metadata(path: Path) -> dict:
    m = FILENAME_RE.match(path.name)
    if not m:
        raise ValueError(f"Unexpected filename format for pibcie: {path.name}")
...
def parse_pibcie_file(path: Path) -> pd.DataFrame:
    ...
    with path.open("r", encoding="latin-1", errors="replace") as f:
        for i, raw_line in enumerate(f):
            ...
            if i == 0:
                if line != "PIBCIE;":
                    raise ValueError(f"{path.name}: expected first line 'PIBCIE;', got {line!r}")
                continue
            ...
            data_row_counter += 1
            yyyy, mm, dd, period, session_number, unit_code, assigned_power, grupo, offer_type = parts
    ...
    for path in files:
            ...
            summary_rows.append(
                {
                    ...
                    "rows_output": 0,
                    "output_path": "",
                    "error_message": "Filename does not match pibcie pattern",
                }
            )
            continue
        ...
        try:
            ...
            row = {
                ...
                "file_family": "pibcie",
                "filename": path.name,
                "parser_name": "mtu.parsing.pibcie.parse_pibcie_file:v1",
