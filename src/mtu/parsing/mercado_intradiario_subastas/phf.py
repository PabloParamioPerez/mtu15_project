...
FILENAME_RE = re.compile(r"^phf_(\d{8})(\d{2})\.(\d+)$")


def parse_filename_metadata(path: Path) -> dict:
    m = FILENAME_RE.match(path.name)
    if not m:
        raise ValueError(f"Unexpected filename format for phf: {path.name}")
...
def parse_phf_file(path: Path) -> pd.DataFrame:
    ...
    with path.open("r", encoding="latin-1", errors="replace") as f:
        for i, raw_line in enumerate(f):
            ...
            if i == 0:
                if line != "PHF;":
                    raise ValueError(f"{path.name}: expected first line 'PHF;', got {line!r}")
                continue
            ...
            data_row_counter += 1
            yyyy, mm, dd, period, session_number, unit_code, assigned_power, flag_etiqueta, offer_type = parts
            ...
            rows.append(
                {
                    ...
                    "flag_etiqueta": int(flag_etiqueta),
    ...
    for path in files:
            ...
            summary_rows.append(
                {
                    ...
                    "rows_output": 0,
                    "output_path": "",
                    "error_message": "Filename does not match phf pattern",
                }
            )
            continue
        ...
        try:
            ...
            row = {
                ...
                "file_family": "phf",
                "filename": path.name,
                "parser_name": "mtu.parsing.mercado_intradiario_subastas.phf.parse_phf_file:v1",
