...
FILENAME_RE = re.compile(r"^pibci_(\d{8})(\d{2})\.(\d+)$")


def parse_filename_metadata(path: Path) -> dict:
    m = FILENAME_RE.match(path.name)
    if not m:
        raise ValueError(f"Unexpected filename format for pibci: {path.name}")
...
def parse_pibci_file(path: Path) -> pd.DataFrame:
    ...
    with path.open("r", encoding="latin-1", errors="replace") as f:
        for i, raw_line in enumerate(f):
            ...
            if i == 0:
                if line != "PIBCI;":
                    raise ValueError(f"{path.name}: expected first line 'PIBCI;', got {line!r}")
                continue
            ...
            summary_rows.append(
                {
                    ...
                    "rows_output": 0,
                    "output_path": "",
                    "error_message": "Filename does not match pibci pattern",
                }
            )
            continue
