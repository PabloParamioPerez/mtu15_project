"""Parse ENTSO-E actual generation per production type (A75).

Same XML skeleton as A69 (`GL_MarketDocument > TimeSeries > Period > Point`).
One TimeSeries per `psrType` (B16 solar, B18 wind offshore, B19 wind onshore).
Point carries `quantity` in MW at the TimeSeries resolution. The difference
vs A69 is semantic: A75 is *realised* generation, A69 is *day-ahead forecast*.

Output columns match `wind_solar_forecast.py` so forecast-error computation
is a simple join on (isp_start_utc, psr_type):
