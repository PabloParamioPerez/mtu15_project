...
FILENAME_RE = re.compile(r"^pdbc_(\d{8})\.(\d+)$")


def parse_filename_metadata(path: Path) -> dict:
    m = FILENAME_RE.match(path.name)
    if not m:
        raise ValueError(f"Unexpected filename format for pdbc: {path.name}")
...
def parse_pdbc_file(path: Path) -> pd.DataFrame:
    ...
    with path.open("r", encoding="latin-1", errors="replace") as f:
        for i, raw_line in enumerate(f):
            ...
            if i == 0:
                if line != "PDBC;":
                    raise ValueError(f"{path.name}: expected first line 'PDBC;', got {line!r}")
                continue
            ...
            yyyy, mm, dd, period, unit_code, assigned_power, unused_zero, offer_type, offer_number = parts
    ...
    dup_mask = df.duplicated(subset=["date", "period", "unit_code"])
    if dup_mask.any():
        dups = (
            df.loc[dup_mask, ["date", "period", "unit_code"]]
            .drop_duplicates()
            .sort_values(["date", "period", "unit_code"])
        )
        dups_preview = [tuple(x) for x in dups.head(10).to_numpy().tolist()]
        more = "..." if len(dups) > 10 else ""
        raise ValueError(f"{path.name}: duplicated (date, period, unit_code) rows: {dups_preview}{more}")

    mtu_minutes = infer_mtu_minutes_from_periods(df["period"])
    validate_period_values(path, df["period"], mtu_minutes)

    df = df.sort_values(["date", "period", "unit_code"]).reset_index(drop=True)
    ...
    for path in files:
            ...
            summary_rows.append(
                {
                    ...
                    "rows_output": 0,
                    "output_path": "",
                    "error_message": "Filename does not match pdbc pattern",
                }
            )
            continue
        ...
        try:
            ...
            row = {
                ...
                "file_family": "pdbc",
                "filename": path.name,
                "parser_name": "mtu.parsing.pdbc.parse_pdbc_file:v1",
