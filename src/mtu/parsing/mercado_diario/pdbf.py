"""Parse OMIE PDBF files (Programa Diario Básico de Funcionamiento).

PDBF is per-unit cleared output AFTER bilateral-contract executions are added
to PDBC. Format per OMIE Spec §5.1.2.3 (v1.37):

  Header line:    PDBF;
  Data line:      yyyy;mm;dd;period;unit_code;potencia_mw;IdCBF;tipo_oferta;NumOf;
  Last line:      *

Fields (9 per data row, semicolon-separated):
  year (I4), month (I2), day (I2), period (I3, 1..100),
  unit_code (A7),
  assigned_power_mw (F7.1, signed -99999.9..99999.9),
  bilateral_contract_id (I8, EMPTY when not a bilateral),
  offer_type (I2, 0..99 — value "4" indicates bilateral execution),
  offer_number (I8, -1..99999999).

Difference from PDBC: field 7 in PDBC is `unused_zero` (always 0); in PDBF it
is the bilateral contract identifier (empty for market-cleared rows).
"""
...
FILENAME_RE = re.compile(r"^pdbf_(\d{8})\.(\d+)$")


def parse_filename_metadata(path: Path) -> dict:
    m = FILENAME_RE.match(path.name)
    if not m:
        raise ValueError(f"Unexpected filename format for pdbf: {path.name}")
...
def parse_pdbf_file(path: Path) -> pd.DataFrame:
    ...
    with path.open("r", encoding="latin-1", errors="replace") as f:
        for i, raw_line in enumerate(f):
            ...
            if i == 0:
                if line != "PDBF;":
                    raise ValueError(f"{path.name}: expected first line 'PDBF;', got {line!r}")
                continue
            ...
            yyyy, mm, dd, period, unit_code, assigned_power, bilateral_id, offer_type, offer_number = parts

            # bilateral_contract_id is empty for market-cleared rows; integer for bilaterals
            bilateral_id_clean = bilateral_id.strip()
            bilateral_contract_id = int(bilateral_id_clean) if bilateral_id_clean else None
            ...
            rows.append(
                {
                    ...
                    "bilateral_contract_id": bilateral_contract_id,
    ...
    # PDBF can have multiple rows per (date, period, unit_code) when a unit has
    # both market-cleared (offer_type != 4) and bilateral (offer_type == 4) entries.
    # Uniqueness is on (date, period, unit_code, offer_type, offer_number).
    dup_mask = df.duplicated(subset=["date", "period", "unit_code", "offer_type", "offer_number"])
    if dup_mask.any():
        dups = (
            df.loc[dup_mask, ["date", "period", "unit_code", "offer_type", "offer_number"]]
            .drop_duplicates()
            .sort_values(["date", "period", "unit_code", "offer_type", "offer_number"])
        )
        dups_preview = [tuple(x) for x in dups.head(10).to_numpy().tolist()]
        more = "..." if len(dups) > 10 else ""
        raise ValueError(
            f"{path.name}: duplicated (date, period, unit_code, offer_type, offer_number) "
            f"rows: {dups_preview}{more}"
        )

    mtu_minutes = infer_mtu_minutes_from_periods(df["period"])
    validate_period_values(path, df["period"], mtu_minutes)

    df = df.sort_values(["date", "period", "unit_code", "offer_type", "offer_number"]).reset_index(drop=True)
    ...
    for path in files:
            ...
            summary_rows.append(
                {
                    ...
                    "rows_output": 0,
                    "output_path": "",
                    "error_message": "Filename does not match pdbf pattern",
                }
            )
            continue
        ...
        try:
            ...
            row = {
                ...
                "file_family": "pdbf",
                "filename": path.name,
                "parser_name": "mtu.parsing.pdbf.parse_pdbf_file:v1",
