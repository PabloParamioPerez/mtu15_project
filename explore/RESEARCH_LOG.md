...
| **H18** | Big-4 Pumped Hydro uses iceberg orders to hide XBID strategic positioning, increasingly post-reform | [SUPPORTED — descriptive] | `nb09 §8a`. Iceberg rate = fraction of XBID sell orders with `exec_condition='ICE'` (visible ~5% of true qty on avg for pumped hydro). Big-4 Pumped Hydro: 40% (6-sess) → 47% (3-sess) → 60% (ISP15) → 58% (DA60/ID15) → **98.8% (DA15/ID15 post-MTU15-DA)**. Near-total iceberg usage after MTU15-DA. Also striking: Fringe CCGT iceberg rate jumps from 17-37% pre-reform to 78-87% post-MTU15-IDA (Fringe shift is larger than Big-4 CCGT's 3% → 7%). Reform-correlated, tech-specific, sharpest shift in nb09 so far. Placebo-localisation not tested. |
...
| [09_bid_shape_eda.ipynb](09_bid_shape_eda.ipynb) | 26 | Bid-structure EDA. §1-§2 DA tranche counts and monthly series; §3-§4 IDA tranches + DA+IDA overlay (H12); §5 IDA tranche-price range (H13); §6 IDA supply-function slope (H14, Chang 2026-style); §7 within-hour bid similarity (H15); §8a XBID iceberg rates (H18) + §8b storage trajectory (H16). | H10 Big-4 CCGT DA tranches drops 5-7 → 1-2 at MTU15-IDA. H12 SUPPORTED, H13 REJECTED, H14 SUPPORTED. H15 REJECTED for most cells: 80-99% of offer-hours have identical bids across 4 ISPs. H18 SUPPORTED: Big-4 PumpedHydro iceberg 40% → 99% across reforms; Fringe CCGT 17-37% → 78-87%. H16 FLAGGED: storage activity jumped 300× post-MTU15-IDA but without iceberg use. |
