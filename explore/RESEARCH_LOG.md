...
| **H18** | Big-4 Pumped Hydro uses iceberg orders to hide XBID strategic positioning, increasingly post-MTU15-DA | **[SUPPORTED — descriptive + placebo-validated]** | `nb09 §8a`, `§9`. Iceberg rate = fraction of XBID sell orders with `exec_condition='ICE'` (visible ~5% of true qty on avg for pumped hydro). Big-4 Pumped Hydro: 40% (6-sess) → 47% (3-sess) → 60% (ISP15) → 58% (DA60/ID15) → **98.8% (DA15/ID15 post-MTU15-DA)**. **§9 PLACEBO: survives sliding-boundary test at monthly resolution.** Real MTU15-DA Δ = 0.480, non-reform placebo Δ range [0.210, 0.327], empirical p = 0/18 = 0.000 (zero non-reform boundaries match). Monthly series: volatile 24-92% pre-Oct-2025, saturated 97-100% from Oct 2025 onwards. **First placebo-validated finding in the entire project** (H5 DiD failed, H7 GE×CCGT flip failed). Also observed: Fringe CCGT iceberg rate 17-37% → 78-87% post-MTU15-IDA (not yet placebo-tested). |
...
| [09_bid_shape_eda.ipynb](09_bid_shape_eda.ipynb) | 29 | Bid-structure EDA. §1-§2 DA tranche counts; §3-§4 IDA tranches (H12); §5 IDA price-range (H13); §6 supply-function slope (H14); §7 within-hour bid similarity (H15); §8 XBID iceberg + storage trajectory (H18, H16); §9 placebo test for H18. | H10/H12/H14/H16/H18 SUPPORTED, H13/H15 REJECTED. **§9: H18 placebo-validated** (empirical p=0/18, sliding-boundary test). This is the project's first placebo-survived finding. |
