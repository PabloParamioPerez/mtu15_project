...
% hour-class identification strategy. Companion: additional_results.tex.
% ============================================================================
...
\phantomsection\label{sec:glossary}
...
\begin{itemize}\setlength{\itemsep}{1pt}
  \item $\bar p_{t,m,d,\mathcal{X}}$: MW-weighted mean of in-band tranche prices, aggregated across units of tech $t$ and hours in class $\mathcal{X}$ on date $d$ in market $m$ (the cell-level counterpart of the per-curve $\bar p_{u,d,p}$ defined in \hyperref[sec:setup]{\S~1}).
...
\subsection*{3.D. Out of scope: full-curve counterfactual via functional factor regression}
\addcontentsline{toc}{subsection}{3.D. Out of scope: full-curve counterfactual via functional factor regression}\phantomsection\label{sec:fbr}
...
Both DA and IDA clearing prices drop sharply under ID15 (Table above; top row of Figure~\ref{fig:bsts-id15}). The cross-market symmetry is the signature: both markets drop by similar magnitudes on the same day even though ID15 only changed \emph{intraday} granularity --- the rational-anticipation channel into day-ahead. The BSTS conditions on wind, solar, and gas, so the drop is net of the merit-order shift; the pre-window in-sample fit is tight. \emph{Verdict.} The 2024 same-calendar placebo also runs negative; the joint placebo-net 95\% interval brackets zero, so the level effect is not retained. A cross-border control reads the drop as Spain-specific: the Spain$-$France hourly DA-price gap widens against Spain across the 40-day pre-blackout MTU15-IDA window --- France is on the same SIDC mechanism but does not show the same drop --- then reverts once both continental MTU15 reforms are in and reforzada has settled.\footnote{Spain$-$France hourly DA-price gap (EUR/MWh): $-4.27$ in ISP15-win $\to$ $-21.51$ in MTU15-IDA pre-blk (a $\sim\!17$~EUR/MWh Spain-specific shift) $\to$ $-8.48$ in DA15/ID15; detail in \emph{additional results} \S 9.4.} We report the raw drop, its cross-market signature, and this cross-border attribution. DA15 price effects are small and not directional.

\emph{ID15 CCGT cleared.} CCGT IDA shows a tiny drop under ID15 that does not survive the placebo net (Table~\ref{tab:bsts-spec-a}, placebo-net $+1.10$), and the DA side does not move either --- but the latter is a floor effect, not a null: by the MTU15-IDA window CCGT DA-cleared volume is already near zero (the $q_1$ drop, \emph{additional results} \S 6), so even if granularity gave CCGT an instrument to shift out of DA there is essentially no room to do so.
...
\section*{Appendix B. Continuous-market response at the three structural reforms}
