...
- `scripts/analysis/{system,firm,regulatory,balancing,bid,modelling,panels,attic}/` — empirical analyses, organised by topic (system = Acts I friction; firm = Acts II Big-4 strategic; bid = bid-shape and granularity; regulatory = RT2 + CNMC; balancing = aFRR/mFRR/nuclear-availability). The `attic/` subfolder holds retired analyses (lerner/, synthetic/, firm/-dead and other pre-pivot scripts moved 2026-05-04). **Replication consolidation 2026-06-17:** every analysis script that does NOT produce a thesis exhibit was moved into `scripts/analysis/attic/<topic>/` (≈280 scripts — exploratory, robustness-superseded, and dead-branch work). The live (non-attic) `scripts/analysis/` tree is now exactly the set the replication driver runs and the paper cites; the authoritative exhibit→script map is `REPLICATION.md`, and `replication/run_replication.py` runs them section by section. Never move a script referenced in `REPLICATION.md`, `thesis/paper/thesis.tex` (`% Source:` / figure includes), or the driver into the attic.
